...
#define TINFO_SYM(rc, fn, params, args, ret) \
  ...
  extern "C" rc fn params { ret g_TINFO_##fn args; }

TINFO_SYM(char *, tgoto, (char *string, int x, int y), (string, x, y), return);
TINFO_SYM(int, tputs, (const char *str, int affcnt, int (*putc)(int)),
          (str, affcnt, putc), return);
...
static bool init_tinfo_functions() {
  ...
  if (!s_ncurses_handle) {
    // Long time ago, ncurses was two libraries. So if libtinfo fails, try
    // libncurses.
...
#define LOADTINFOFUNC(_handle, _func, _failed)                    \
  do {                                                            \
    ...
    if (!g_TINFO_##_func) _failed = true;                         \
  } while (0)
...
static unsigned char editline_complete(EditLine *el,
                                       int ch __attribute__((__unused__))) {
  static const char *s_cmds[] = {
      "cvarlist ",         "find ",   "help ",          "maps ",
      "nextlevel",         "quit",    "status",         "sv_cheats ",
      "tf_bot_quota ",     "toggle ", "sv_dump_edicts",
#ifdef STAGING_ONLY
      "tf_bot_use_items ",
#endif
  };
  ...
  if (len > 0) {
    for (int i = 0; i < ssize(s_cmds); i++) {
      if (len > strlen(s_cmds[i])) continue;
...
static bool add_command(const char *cmd, int cmd_len) {
  if (cmd) {
    ...
    if (cmd_len > 0) {
      ...
      for (int i = 0; i < 6; i++) {
        while (g_ProcessingCommands) usleep(500);
      }
...
bool CTextConsoleUnix::Init() {
  ...
  if (isatty(STDIN_FILENO)) {
    ...
    if (!init_tinfo_functions()) terminal_threadproc = fgets_threadproc;

    if (pthread_create(&g_threadid, NULL, terminal_threadproc, (void *)m_tty) !=
        0) {
      g_threadid = (pthread_t)-1;
...
char *CTextConsoleUnix::GetLine(int index, char *buf, size_t buflen) {
  if (g_threadid != (pthread_t)-1) {
