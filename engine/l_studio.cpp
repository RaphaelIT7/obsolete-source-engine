...
public:
	...
	inline vertexFileHeader_t *CacheVertexData() { return g_pMDLCache->GetVertexData( (MDLHandle_t)(intp)m_pStudioHdr->virtualModel&0xffff ); }
	...
	DrawModelInfo_t &drawInfo, const ModelRenderInfo_t &pInfo, int drawFlags )
{
	...
	if ( g_pMaterialSystemConfig->nFullbright == 1 )
	{
		...
		static thread_local Vector white[6] = 
		{
			Vector( 1.0, 1.0, 1.0 ),
			Vector( 1.0, 1.0, 1.0 ),
			Vector( 1.0, 1.0, 1.0 ),
...
bool CModelRender::UpdateStaticPropColorData( IHandleEntity *pProp, ModelInstanceHandle_t handle )
{
	...
	if ( !bDebugColor )
	{
		...
		vertexFileHeader_t *pVertexHdr = g_pMDLCache->GetVertexData( (MDLHandle_t)(intp)pStudioHdr->virtualModel&0xffff );
		if ( !pVertexHdr )
		{
...
ModelInstanceHandle_t CModelRender::CreateInstance( IClientRenderable *pRenderable, LightCacheHandle_t *pCache )
{
	...
	// ToDo: Unlock it after setting instance.m_FirstShadow = so that RecomputeStaticLighting though the entire light cache would need to be made threadsafe for that.
...
bool CModelRender::GetBrightestShadowingLightSource( const Vector &vecCenter, Vector& lightPos, Vector& lightBrightness, bool bAllowNonTaggedLights )
{
	...
	static const Vector colorToGray( 0.3f, 0.59f, 0.11f );
